...
namespace Resources
{
    public class ResourceManager
    {
        ...
        {
            byte[] data;
            using (System.IO.Stream stm = typeof(ResourceManager).Assembly.GetManifestResourceStream(string.Format("{0}.Icons.{1}.ico", typeof(ResourceManager).Namespace, Name)))
            {
                if (stm == null) return null;
        ...
        {
            byte[] data;
            using (System.IO.Stream stm = typeof(ResourceManager).Assembly.GetManifestResourceStream(string.Format("XBMCNotify.{0}.Logo.{1}.png", typeof(ResourceManager).Namespace, Name)))
            {
                if (stm == null) return null;
                data = new byte[stm.Length];
                stm.Read(data, 0, data.Length);
