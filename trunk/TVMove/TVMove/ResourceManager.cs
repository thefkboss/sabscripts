...
namespace Resources
{
    public class ResourceManager
    {
        ...
        {
            byte[] data;
            using (System.IO.Stream stm = typeof(ResourceManager).Assembly.GetManifestResourceStream(string.Format("TVMove.{0}.Logo.{1}.png", typeof(ResourceManager).Namespace, Name)))
            {
                if (stm == null) return null;
                data = new byte[stm.Length];
                stm.Read(data, 0, data.Length);
