...
namespace SABSync
{
    public class ConfigOld
    {
        ...
        private static Configuration _cfg = ConfigurationManager.OpenExeConfiguration(@"SABSync.exe");

        public ConfigOld() : this(ConfigurationManager.AppSettings)
        {
        }

        public ConfigOld(NameValueCollection settings)
        {
            Settings = settings;

            DownloadPropers = Convert.ToBoolean(Settings["downloadPropers"] ?? "false");
            Interval = Convert.ToInt32(ConfigurationManager.AppSettings["interval"] ?? "15");
